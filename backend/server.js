...
app.get('/', (req, res) => {
    res.send({
        activeStatus: true,
        error: false,
        message: "Video call server is running." // Added message for clarity
    });
});

const server = http.createServer(app);

// Use a more specific origin for production (best practice)
const io = new Server(server, {
    cors: {
        // IMPORTANT: Use process.env.CLIENT_URL or a specific URL in production
        origin: "*", 
        methods: ["GET", "POST"]
    }
});
...
io.on('connection', (socket) => {
    ...
    // --- Utility function for finding the user's current room (Fix for disconnect) ---
    ...
    socket.on('ice-candidate', ({ to, candidate, from }) => {
        ...
        // Ensure candidate is not null before emitting (minor safety check)
        if (candidate) {
    ...
    socket.on('send-message', ({ roomId, message }) => {
        ...
        // Add sender ID to message for frontend display
    ...
    socket.on('toggle-camera', ({ roomId, isCameraOn }) => {
        ...
        const room = rooms.get(roomId);
        if (room?.has(socket.id)) { // Use optional chaining for cleaner check
    ...
    // Disconnect event (user closes browser, loses connection)
    socket.on('disconnect', () => {
        console.log(`[DISCONNECT] User disconnected: ${socket.id}`);
        // FIX: Only check the room the user was actually in
    ...
    function handleUserLeave(socket, roomId) {
        ...
        if (room) {
            ...
            // Only send the user-left event if a user object was successfully retrieved/deleted
            if (user) {
...
// Use a dynamic port (Best Practice for deployment)
